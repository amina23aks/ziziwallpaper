...
export function AdminTopBar({
  ...
  return (
    <>
      ...
      <header className="fixed inset-x-0 top-0 z-50 border-b border-[color:var(--app-border)] bg-[var(--app-surface)] text-[var(--app-text)] md:hidden">
        <div className="flex min-h-14 items-center gap-3 px-4 py-2 [direction:ltr]">
          {leading ? (
            ...
            <Link
              href={backHref}
              className="inline-flex shrink-0 items-center justify-center px-1 text-xl leading-none text-[var(--app-text-muted)] transition hover:text-[var(--app-text)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400/40"
              aria-label="رجوع"
            >
              ←
            </Link>
          ...
          <div className="min-w-0 flex-1 [direction:rtl] text-right">
            <h1 className="truncate text-base font-bold text-[var(--app-text)]">{title}</h1>
            {subtitle ? <p className="truncate text-xs text-[var(--app-text-muted)]">{subtitle}</p> : null}
          </div>
